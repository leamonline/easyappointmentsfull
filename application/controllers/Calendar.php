    ...
    public function save_appointment(): void
    {
        try {
            ...
            if ($appointment_data) {
                ...
                // Salon capacity validation (if salon mode is enabled).
                if ($this->salon_capacity->is_enabled() && empty($appointment['is_unavailability'])) {
                    ...
                    if (!empty($appointment['id_pets'])) {
                        try {
                            $pet = $this->pets_model->find((int) $appointment['id_pets']);
                            $pet_size = $pet['size'] ?? 'small';
                        } catch (Throwable $e) {
                            // Pet not found, default to small.
                        }
                    }
                    ...
                    if (!$slot_result['available']) {
                        $alternatives = $this->salon_capacity->get_alternative_slots(
                            $date,
                            $time,
    ...
    public function get_calendar_appointments_for_table_view(): void
    {
        try {
            ...
            foreach ($response['appointments'] as &$appointment) {
                ...
                $appointment['pet'] = !empty($appointment['id_pets'])
                    ? ($this->db->get_where('pets', ['id' => $appointment['id_pets']])->row_array() ?: null)
                    : null;
                $appointment['customer_pets'] = $this->pets_model->get_by_customer($appointment['id_users_customer']);
            }
