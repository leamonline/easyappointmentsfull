...
class Appointments_model extends EA_Model
{
    ...
    public function validate(array $appointment, array $options = []): void
    {
        ...
        if (
            ...
            throw new InvalidArgumentException('Not all required fields are provided: ' . json_encode(array_keys($appointment)));
        }
    ...
    public function load(array &$appointment, array $resources): void
    {
        ...
        foreach ($resources as $resource) {
            switch ($resource) {
                ...
                case 'pet':
                    $pet_id = $appointment['id_pets'] ?? ($appointment['petId'] ?? null);
                    $appointment['pet'] = $pet_id
                        ? $this->db->get_where('pets', ['id' => $pet_id])->row_array()
                        : null;
                    break;
