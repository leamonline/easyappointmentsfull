    ...
    function addEventListeners() {
        ...
        $calendarPage.on('click', '.edit-popover', (event) => {
            ...
            if (data.hasOwnProperty('workingPlanException')) {
                ...
                // Populate pet selector with customer's pets.
                const $selectPet = $appointmentsModal.find('#select-pet');
                $selectPet.empty().append(new Option('-- Select Dog --', ''));
                if (appointment.customer_pets && appointment.customer_pets.length) {
                    appointment.customer_pets.forEach((pet) => {
                        const label = `${pet.name} (${pet.breed || 'Unknown breed'}) - ${pet.size}`;
                        $selectPet.append(new Option(label, pet.id));
                    });
                }
                if (appointment.id_pets) {
                    $selectPet.val(appointment.id_pets);
                }
        ...
        App.Http.Calendar.getCalendarAppointments(recordId, startDate, endDate, filterType)
            .done((response) => {
                ...
                response.appointments.forEach((appointment) => {
                    ...
                    // Salon mode: "Dog Name (Breed) — Owner Name"
                    if (appointment.pet && appointment.pet.name) {
                        const petInfo = appointment.pet.breed
                            ? `${appointment.pet.name} (${appointment.pet.breed})`
                            : appointment.pet.name;

                        const customerInfo = [];
                        if (appointment.customer.first_name) customerInfo.push(appointment.customer.first_name);
                        if (appointment.customer.last_name) customerInfo.push(appointment.customer.last_name);

                        title = customerInfo.length
                            ? `${petInfo} — ${customerInfo.join(' ')}`
                            : petInfo;
                    } else {
                        // Fallback: original format "Service - Customer"
                        const titleParts = [appointment.service.name];
                        const customerInfo = [];
                        if (appointment.customer.first_name) customerInfo.push(appointment.customer.first_name);
                        if (appointment.customer.last_name) customerInfo.push(appointment.customer.last_name);
                        if (customerInfo.length) titleParts.push(customerInfo.join(' '));
                        title = titleParts.join(' - ');
                    }

                    // Salon colour coding: by dog size, deposit status, or service.
                    let eventColor = appointment.color;

                    if (appointment.pet) {
                        const sizeColors = {small: '#4CAF50', medium: '#2196F3', large: '#FF9800'};
                        eventColor = sizeColors[appointment.pet.size] || eventColor;

                        // Override with deposit status colour if awaiting.
                        if (appointment.customer.deposit_status === 'awaiting') {
                            eventColor = '#F44336'; // Red for awaiting deposit.
                        }
                    }
